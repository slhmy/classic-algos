...
void placePivotAtEndNothing(int arr[], int left_bound_idx, int right_bound_idx)
{
    // Do nothing
}
...
int partition(int arr[], int left_bound_idx, int right_bound_idx)
{
    ...
    // Prepare a index pointer to store the last element that is smaller than the pivot
    int left_arr_ptr = left_bound_idx - 1;
    // Find the elements that are smaller than the pivot and move them to the left part
    for (int i = left_bound_idx; i < right_bound_idx; i++)
        if (arr[i] < pivot)
        {
            // The worst case is that left_arr_ptr == i
            // Otherwise, left_arr_ptr < i, which means arr[left_arr_ptr] >= pivot
...
void quickSort(int arr[], int left_bound_idx, int right_bound_idx)
{
    ...
    if (left_bound_idx < right_bound_idx)
    {
        ...
        // Pivot don't need to be moved, since it's already in the right position
