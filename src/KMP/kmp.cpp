...
int KMP(const string& m_str, const string& t_str)
{
    ...
    while (mPtr < m_str.length() && tPtr < m_str.length()) {
        if (tPtr == -1 || m_str[mPtr] == t_str[tPtr]) mPtr++, tPtr++;
        // when mismatch occurs, you try another match possibility
        // by right move the target_str, aligning m_str[mPtr-1] to a previous matched t_str character
        // Ex.
        // |abbaabbaaba|                    |abbaabbaaba|
        //        ^                                ^
        // |abbaaba    |           -->      |    abbaaba|
    ...
    // answer result
    if (tPtr == t_str.length()) return mPtr - t_str.length();
    else return -1;
}

int main() {
    cout << KMP("abbaabbaaba", "abbaaba") << endl;
}
