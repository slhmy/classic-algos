...
void RBTree<T>::fixDeleteRBTree(Node<T> *&root, Node<T> *&pt) {
    ...
    if (getColor(pt) == RED || getColor(pt->left) == RED || getColor(pt->right) == RED) {
...
template <typename T>
void RBTree<T>::inorder() { inorderHelper(root); }

template <typename T>
void RBTree<T>::levelOrder() { levelOrderHelper(root); }
...
template <typename T>
void RBTree<T>::levelOrderHelper(Node<T> *root) {
    if (root == nullptr)
        return;
